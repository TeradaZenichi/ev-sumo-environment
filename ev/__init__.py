...
class EV:
    def __init__(self, id:str,type:str,dest:str, total_length):
        ...
        self.actions_dic = {
            ...
            1: self.action.newdestin,
            2: self.action.newroute,
            3: self.action.newroute_finaldest,
            4: self.action.returnfinaldest,
            5: self.action.skip_stop,
            6: self.action.stopParking,
            7: self.action.recharge_substation}
    ...
    class Actions : 
        ...
        def recharge_substation(self,dest):                                              # dest vector = [station edge, station id]
            ...
            traci.vehicle.setChargingStationStop(ev.id, dest[1], duration=300,flags=1)
            return 
        
        def stopParking(self,dest):                                                      # dest vector = [parking edge, parking_id]
            ev = self.ev
            traci.vehicle.changeTarget(ev.id, dest[0])
            traci.vehicle.setParkingAreaStop(ev.id, dest[1], duration=300)
            return
        ...
        def newroute(self,dest):                                                         # destination vector = [destination id]
            ...
            if ev.penultimate_dest != ev.final_dest:
        ...
        def newroute_finaldest(self,dest): # destination vector = [destination id]. 
            ...
            route = traci.simulation.findRoute(ev.edge, dest[0], vType=ev.type)
